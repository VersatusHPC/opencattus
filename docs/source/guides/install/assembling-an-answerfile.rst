...
``latest-3.2-LTS`` targets the current LTS baseline, ``latest-2.9-LTS`` keeps
the older legacy LTS lane, and ``latest`` tracks the current non-LTS
major-version repo. OpenCATTUS keeps those behaviors explicit instead of
assuming every Enterprise Linux release should use the same DOCA layout.
