/*
 * Copyright 2022 Vinícius Ferrão <vinicius@ferrao.net.br>
 * SPDX-License-Identifier: Apache-2.0
 */

#ifndef OPENCATTUS_PRESENTER_H_
#define OPENCATTUS_PRESENTER_H_

#include <opencattus/models/cluster.h>
#include <opencattus/models/pbs.h>
#include <opencattus/models/queuesystem.h>
#include <opencattus/models/slurm.h>
#include <opencattus/services/log.h>
#include <opencattus/view/newt.h>

namespace opencattus::presenter {

using IRunner = opencattus::services::IRunner;
using Cluster = opencattus::models::Cluster;
using QueueSystem = opencattus::models::QueueSystem;
using SLURM = opencattus::models::SLURM;
using PBS = opencattus::models::PBS;
using OS = opencattus::models::OS;
using CPU = opencattus::models::CPU;
using Node = opencattus::models::Node;

class Presenter {
protected:
    // FIXME: According to Herb Sutter we shouldn't be passing this as const
    // ref&
    //  - https://youtu.be/xnqTKD8uD64?t=1145
    //  - http://tinyurl.com/gotw91
    std::unique_ptr<Cluster>& m_model;
    std::unique_ptr<Newt>& m_view;

    struct Messages {
        struct Placeholder {
            static constexpr const auto help
                = "We're sorry, but there's no help available for this session";
        };
    };

    Presenter(std::unique_ptr<Cluster>& model, std::unique_ptr<Newt>& view)
        : m_model(model)
        , m_view(view)
    {
    }

    template <class C, typename... Args> void Call(Args&&... args)
    {
        C(m_model, m_view, std::forward<Args>(args)...);
    }
};

};

#endif // OPENCATTUS_PRESENTER_H_
