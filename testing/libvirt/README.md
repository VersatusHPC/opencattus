...
`opencattus-el9-lab.sh` therefore installs an exit trap that re-owns every path under `WORKSPACE` to `SUDO_UID:SUDO_GID` before exiting. The trap fires on success, failure, and `SIGHUP`/`SIGINT`/`SIGTERM` (Jenkins timeouts send `SIGTERM`; a `SIGKILL` cannot be intercepted). It stays inert unless all of these hold: `WORKSPACE` is set to an existing directory other than `/`, the harness runs as root, and `SUDO_UID`/`SUDO_GID` identify a non-root invoking user — so unprivileged GitHub Actions runs and manual root sessions without sudo are unaffected. The re-own uses `chown -h`, so symlinks under the workspace never change the ownership of their targets.
